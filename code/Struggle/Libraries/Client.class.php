    ...
    private function _buildJs($content, $isFile, $pos){
        ...
        if($isFile){
            if(($iQuesPos = strrpos($content,'?'))!==false){
                ...
                $sQuesAfter = View::self()->replaceGlobalConst($sQuesAfter);
            }
            ...
            if (is_file($js) && is_string($js)){
                ...
                View::self()->addJs("<script type='text/javascript' src='{$js}{$sQuesAfter}'></script>",$pos);
                return true;
            }
        }else{
            if ($js && is_string($js)){
                //$this->mDynAttr['pager'][$pos][] = "<script type='text/javascript'>\n{$js}\n</script>";
				View::self()->addJs("<script type='text/javascript'>\n{$js}\n</script>",$pos);
                return true;
            }
        }
    ...
    private function _buildCss($content, $isFile, $pos){
        ...
        if($isFile){
            ...
            if (is_file($css) && is_string($css)){
                View::self()->addCss("<link type='text/css' rel='stylesheet' href='{$css}' />",$pos);
                return true;
            }
        }else{
            if ($css && is_string($css)){
                View::self()->addCss("<script type='text/javascript'>\n{$css}\n</script>",$pos);
                return true;
            }
        }
	...
	private function parseUrlFormatStr($str){
	    ...
	    if($aAttr = parse_url($str)){
			...
			if(strtolower($aAttr['scheme']) == 'file'){
				...
				$retval['TYPE_'.strtoupper($aFileAttr[0])][$iPos] = $tmp;
			}
		}else{
